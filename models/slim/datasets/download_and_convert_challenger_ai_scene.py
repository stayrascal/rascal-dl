...
def _get_filenames_and_classes(dataset_dir):
  ...
  return photo_filenames, photo_classes, class_names


def _get_dataset_filename(dataset_dir, split_name, shard_id):
  output_filename = 'challenger_ai_scene_%s_%05d-of-%05d.tfrecord' % (
      split_name, shard_id, _NUM_SHARDS)
  return os.path.join(dataset_dir, output_filename)


def _convert_dataset(split_name, filenames, classes, dataset_dir):
...
def run(dataset_dir):
  ...
  photo_filenames, photo_classes, class_names = _get_filenames_and_classes(dataset_dir)
  class_names_to_ids = dict(zip(class_names, range(len(class_names))))

  # Divide into train and test:
  random.seed(_RANDOM_SEED)
  random_list = list(range(len(photo_filenames)))
