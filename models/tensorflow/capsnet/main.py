...
def main(_):
    ...
    with sv.managed_session(config=config) as sess:
        num_batch = int(6000 / cfg.batch_size)
        num_test_batch = 1000 // cfg.batch_size
                ...
                if (global_step + 1) % cfg.test_sum_freq == 0:
                    ...
                    for i in range(num_test_batch):
                        ...
                        test_acc = sess.run(capsNet.batch_accuracy,
                                            {capsNet: teX[start:end], capsNet.labels: teY[start:end]})
                    test_acc = test_acc / (cfg.batch_size * num_test_batch)
                    fd_results.write(str(global_step + 1) + ',' + str(test_acc) + '\n')
                    fd_results.flush()
            if epoch % cfg.save_freq == 0:
                sv.saver.save(sess, cfg.logdir + '/model_epoch_%04d_%03d' % (epoch, global_step))
    fd_results.close()
